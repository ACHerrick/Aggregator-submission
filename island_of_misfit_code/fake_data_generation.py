...
'''
Ths file was used to generate dummy data for cities, in order to enable testing
before accurate review site data was obtained. Code here generates a list of lists,
which includes randomly generating different #'s of cuisine tags per restaurant, and
places different weights on the average ratings of certain cusiines.

It was never intended to be used in the final product.
'''
...
# Defining possible tags/cities/data sources/cuisines
...
def generate_fake_data(req_count):
    ...
    for i in range(req_count):
        ...
        if cuisine_seed_a < 60:
            cuisine_seed_b = random.randrange(0, 17)
            cuisine_tags.append(cuisine_list[cuisine_seed_b])
        elif cuisine_seed_a < 85:
            cuisine_seed_b = random.randrange(0, 17)
            cuisine_seed_c = random.randrange(0, 17)
            cuisine_tags.append(cuisine_list[cuisine_seed_b])
            ...
            cuisine_seed_b = random.randrange(0, 17)
            cuisine_seed_c = random.randrange(0, 17)
            cuisine_seed_d = random.randrange(0, 17)
            cuisine_tags.append(cuisine_list[cuisine_seed_b])
            if cuisine_seed_c != cuisine_seed_b:
                cuisine_tags.append(cuisine_list[cuisine_seed_c])
...
def calculate_cuisine_score(data, cuisine):
    '''
    This code represented an early attempt to run comparisons directly from
    a list of lists. It is unncessary because all calculations are instead
    done through SQL databases.
    '''
    chinese_a = 0
    chinese_rating_a = []
    chinese_b = 0
