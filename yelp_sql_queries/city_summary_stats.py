...
# Hardcoding the dictionary here allows the aggregator on the website to 
# load results more quickly, as it does not need to read an entire csv on every run.
...
def get_summary_info(city = {}, database = "yelp_raw.db"):
    ...
    If no city is specified, returns results for all cities instead.
